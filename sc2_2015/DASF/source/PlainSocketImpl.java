...
//BEGIN_DASF_MOD
    ...
    @Override protected synchronized InputStream getInputStream() throws IOException {
        checkNotClosed();
		//If medical app return MessageSocketInputStream instead of PlainSocketInputStream
		if(isMedicalApp() == 1)
			return new MessageSocketInputStream(this);
        return new PlainSocketInputStream(this);
    }

	private int isMedicalApp(){
		try {
			java.lang.reflect.Method method = Class.forName("android.os.ServiceManager").getMethod("getService", String.class);
			Object sm = method.invoke(null, "med_dev_init");
			java.lang.reflect.Method method2 = 
					Class.forName("android.os.IMedicalDeviceInitService$Stub").getMethod("asInterface", Class.forName("android.os.IBinder"));
			Object med_dev = method2.invoke(null, sm);
			java.lang.reflect.Method method3 = 
					Class.forName("android.os.IMedicalDeviceInitService$Stub").getMethod("isMedicalAppByUid", (Class[])null);
			boolean res = (Boolean)method3.invoke(med_dev, (Object[])null);
			return res ? 1 : 0;
		}catch(Exception e){}         
		return -1;
    }//isMedicalApp
	...
	private static class MessageSocketInputStream extends InputStream {
		private final PlainSocketImpl socketImpl;
		private int remainingDataLen;
		private byte dataTaint;
		private static final byte dataAppended = (byte)0x40;
		private static final byte MULTIPARTDATA = (byte)0x10;

		private long tOverhead = 0L;//TODO REMOVE TIME TEST
		private long tTotal = 0L;//TODO REMOVE TIME TEST

        public MessageSocketInputStream(PlainSocketImpl socketImpl) {
            this.socketImpl = socketImpl;
			this.remainingDataLen = 0;
			this.dataTaint = 0;
        }
        ...
        @Override public int read() throws IOException {
			...
			return (res != -1) ? buffer[0] & 0xff : -1;
        }

		@Override public int read(byte[] buffer, int offset, int byteCount) throws IOException {
			tOverhead = 0L;//TODO REMOVE TIME TEST
			long tos = 0L;//TODO REMOVE TIME TEST
			long t1 = System.nanoTime();//TODO REMOVE TIME TEST
			if(this.remainingDataLen <= 0){
				//Receive message and parse headers to get data
				tTotal=0L;//TODO REMOVE TIME TEST
				ohTotal = 0L;//TODO REMOVE TIME TEST
				oTotal = 0L;//TODO REMOVE TIME TEST

				dalvik.system.Taint.log("DASF resetting total");
				tos = System.nanoTime();//TODO REMOVE TIME TEST
				byte header = (byte)readSingleByte();			
				tOverhead += (System.nanoTime() - tos);//TODO REMOVE TIME TEST
				if(header == (byte)-1) return -1;
				if(isPolicyHeader(header)){//policy header
					int res = handlePolicy(header);
					if(res == 0) return -1;

					tos = System.nanoTime();//TODO REMOVE TIME TEST
					header = (byte)readSingleByte();
					tOverhead += (System.nanoTime() - tos);//TODO REMOVE TIME TEST

					if(header == -1)throw new IOException("Error getting data header");
				}//if
				//Parse data header				
				if(!isPolicyHeader(header)){//data header

					tos = System.nanoTime();//TODO REMOVE TIME TEST
					int dh2 = readSingleByte();
					int dh3 = readSingleByte();
					int dh4 = readSingleByte();
					tOverhead += (System.nanoTime() - tos);//TODO REMOVE TIME TEST

					int len = getDataLen(header, (byte)dh2, 
										(byte)dh3, 
										(byte)dh4);
					if(len == -1) throw new IOException("Error getting data len");
					this.remainingDataLen = len;
					if(isMultipartDataFlagSet(header)) throw new IOException("Multipart data (Not supported yet)");
					this.dataTaint = getClassificationLevel(header);
				}//get data header
			}//if
			long hTotal = (System.nanoTime()-t1);//TODO REMOVE TIME TEST
			//Read data and add taint (read at most remaining data length) if data len==0 clear taint
			int byteCnt = (byteCount > this.remainingDataLen) ? this.remainingDataLen : byteCount;
			
			tos = System.nanoTime();//TODO REMOVE TIME TEST
			int n = socketImpl.read(buffer, offset, byteCnt);
			tOverhead += (System.nanoTime() - tos);//TODO REMOVE TIME TEST

			if(n == -1) return -1;
			//Taint data
			setDataClassification(buffer, this.dataTaint);
			//Decrement remaining data len (should never be < 0 but just incase)
			this.remainingDataLen -= n;
			if(this.remainingDataLen <= 0){
				this.remainingDataLen = 0;
				this.dataTaint = 0;//data reset taint
			}
			long t2 = System.nanoTime();//TODO REMOVE TIME TEST
			tTotal+=(t2-t1-tOverhead);//TODO REMOVE TIME TEST
			ohTotal+=tOverhead;//TODO REMOVE TIME TEST
			
			oTotal+=(t2-t1);//TODO REMOVE TIME TEST
			 dalvik.system.Taint.log("DASF Read overallTotal ="+oTotal+", overhead total = "+ohTotal+", hook total = "+tTotal);//TODO REMOVE TIME TEST
			
            return n;//return number of bytes read
        }
	private long ohTotal = 0L;//TODO REMOVE TIME TEST
	private long oTotal = 0L;//TODO REMOVE TIME TEST

		private int handlePolicy(byte header) throws IOException{

			long tos = System.nanoTime();//TODO REMOVE TIME TEST
			int b2 = readSingleByte();
			tOverhead += (System.nanoTime() - tos); //TODO REMOVE TIME TEST

			int len = getPolicyLen(header, (byte)b2);
			if(len == -1) throw new IOException("Error getting policy len");
			byte[] policy = new byte[len];
	
			tos = System.nanoTime();//TODO REMOVE TIME TEST
			for(int i = 0; i < len; i++){
				int b = readSingleByte();
				if(b==-1) throw new IOException("Error reading policy");
				policy[i] = (byte)b;
			}//for
			tOverhead += (System.nanoTime() - tos);//TODO REMOVE TIME TEST

			//Pass policy to framework
			int res = passPolicyToFramework(policy);//May want to do something if error
			//If no data return null;
			if(!dataFlagSet(header)) return 0;
			return 1;
		}//handlePolicy

		private int passPolicyToFramework(byte[] policy){
			try {
				java.lang.reflect.Method method = Class.forName("android.os.ServiceManager").getMethod("getService", String.class);
				Object sm = method.invoke(null, "med_dev_init");
				java.lang.reflect.Method method2 = 
				...
				Object med_dev = method2.invoke(null, sm);
				java.lang.reflect.Method method3 = 
						Class.forName("android.os.IMedicalDeviceInitService$Stub").getMethod("enforcePolicy", byte[].class);
				boolean res = (Boolean)method3.invoke(med_dev, policy);
				return res ? 1 : 0;
			}catch(Exception e){}         
			return -1;
		}

		private boolean isPolicyHeader(byte header){
			return (header & 0x80) == 0;
		}//isPolicyHeader
	
		private int getPolicyLen(byte b1, byte b2){
			if(b1 == (byte)-1 || b2 == (byte)-1) return -1;
			return (((int)(b1 & 0x3F)) << 8) ^ ((int)(b2 & 0xFF));
		}//getPolicyLen

		private boolean dataFlagSet(byte header){
			return (header & dataAppended) != 0;
		}//dataFlagSet
	
		private byte getClassificationLevel(byte header){
			return (byte)((header & 0x60) >> 5);
		}//getClassificationLevel
	
		private int getDataLen(byte b1, byte b2, byte b3, byte b4){
			if(b1 == (byte)-1 || b2 == (byte)-1 || b3 == (byte)-1 || b4 == (byte)-1) return -1;
			return (((int)(b1 & 0x0F)) << 24) ^ (((int)(b2 & 0xFF)) << 16) ^ (((int)(b3 & 0xFF)) << 8) ^ ((int)(b4 & 0xFF));
		}//getDataLen

		private static boolean isMultipartDataFlagSet(byte header){
			return (header & MULTIPARTDATA) != 0;
		}
		
		private int setDataClassification(int iVal, byte classificationLevel){
			if(classificationLevel == 1){
				return Taint.addTaintInt(iVal, Taint.TAINT_CLASSIFIED);
			}else if(classificationLevel == 2){
				return Taint.addTaintInt(iVal, Taint.TAINT_SECRET);
			}else if(classificationLevel == 3){
				return Taint.addTaintInt(iVal, Taint.TAINT_TOPSECRET);
			}//else if
			return iVal;
		}

		private void setDataClassification(byte[] data, byte classificationLevel){
			if(classificationLevel == 1){
				Taint.addTaintByteArray(data, Taint.TAINT_CLASSIFIED);
			}else if(classificationLevel == 2){
				Taint.addTaintByteArray(data, Taint.TAINT_SECRET);
			}else if(classificationLevel == 3){
				Taint.addTaintByteArray(data, Taint.TAINT_TOPSECRET);
			}//else if
