...
//BEGIN_DASF_MOD
	try{
		...
		if(!medDevServ.canStartComponent(r.info.applicationInfo.packageName)){
			Log.w("DASF","DASF: realstartActivityLocked (Refusing to start activity)");
			mService.appDiedLocked(app, app.pid, app.thread);
     		requestFinishActivityLocked(r.appToken, Activity.RESULT_CANCELED, null,"2nd-crash");
			return false;
		}
		long t2 = System.nanoTime();
		Log.w("DASF", "realStartActivityLocked TIME "+(t2-t1));
	}catch(Exception e){
		Log.w("DASF", "DASF realstartactivitylocked, we had an exception");	
		return false;
	}//catch
//END_DASF_MOD
