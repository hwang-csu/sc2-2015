...
// begin WITH_TAINT_TRACKING
            ...
            if (tag != Taint.TAINT_CLEAR) {
// BEGIN_DASF_MOD - don't allow file writes for classified and secret data
				if((tag & Taint.TAINT_CLASSIFIED) != 0 || (tag & Taint.TAINT_TOPSECRET) != 0)
					return -1;
// END_DASF_MOD
                int fdInt = fd.getDescriptor();
...
// begin WITH_TAINT_TRACKING
            int tag = buffer.getDirectByteBufferTaint();
            if (tag != Taint.TAINT_CLEAR) {
// BEGIN_DASF_MOD - don't allow socket writes for classified and secret data
				if((tag & Taint.TAINT_SECRET) != 0 || (tag & Taint.TAINT_TOPSECRET) != 0)
					return -1;
// END_DASF_MOD
