...
//BEGIN_DASF_MOD
		//Block sending all classified data through SMS
		int tag = Taint.getTaintString(text);
		if((tag & Taint.TAINT_CLASSIFIED) != 0 || (tag & Taint.TAINT_SECRET) != 0|| (tag & Taint.TAINT_TOPSECRET) != 0){
			return;
		}
//END_DASF_MOD
...
//BEGIN_DASF_MOD
		...
		for(int i = 0; i < parts.size(); i++){
			int tag = Taint.getTaintString(parts.get(i));
			if((tag & Taint.TAINT_CLASSIFIED) != 0 || (tag & Taint.TAINT_SECRET) != 0|| (tag & Taint.TAINT_TOPSECRET) != 0){
				return;
			}
		}
